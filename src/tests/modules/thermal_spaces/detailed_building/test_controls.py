...
def test_space_temperature_control_simple() -> None:
    ...
    # Test no. 1
    estimated_thermal_outputs: Dict[str, float] = (
        space_temperature_control_simple(
            ...
            flow_rates=[],
            space_names=["living_room_1", "kitchen_1"],
        )
    )
    assert estimated_thermal_outputs["living_room_1"] == pytest.approx(
        0.0, abs=0.025
    )
    assert estimated_thermal_outputs["kitchen_1"] == pytest.approx(
        0.0, abs=0.025
    )
    # Test no. 2
    estimated_thermal_outputs: Dict[str, float] = (
