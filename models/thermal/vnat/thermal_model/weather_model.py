...
class Weather:
    ...
    def init_weather(self, epw_file, tz):
...
def import_epw_weather(epf_file):
    ...
    weather_data    = pd.read_csv(main_dir + '/test_cases/' + epf_file, skiprows=8, header=None, names=EPW_vars)
    latitude, longitude = tuple(pd.read_csv(main_dir + '/test_cases/' + epf_file, header=None, nrows=1).loc[:, 6:7].values.flatten().tolist())
