...
class DetailedBuilding(Model):
    ...
    def run(self, time_step: int = 0, n_iteration: int = 0):
        ...
        self.my_T.internal_temperatures_dict = self.my_T.send_to_pressure()  # send temperature values to pressure model

        # Pressure model
        if self.my_P.pressure_model:
            self.my_P.temperatures_update(self.my_T.internal_temperatures_dict)
